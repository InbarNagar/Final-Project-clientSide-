...
import {AllApointemtDetailes,NewAppointmentToClient,Post_SendPushNotification} from "../obj/FunctionAPICode"
import { Alert,Modal, FlatList, Text, View, StyleSheet, TouchableOpacity, Button } from "react-native";
...
const BusinessSchedule = (props) => {
  ...
  const {min,max,businessNumber,booked,isVisible,date,onClose,duration } = props;
  const minNumber = min;
  const maxNumber = max;
  const booked1 = ["8-10", "16-18"];
const startTimes = booked1.map((time) => parseInt(time.split("-")[0]));
          ...
          (result) => {
            ...
            AllApointemtDetailes().then((res) => {
              ...
              if (Number(result.data) === apointment.Number_appointment) {
                console.log("apointment.Number_appointment: "+apointment.Number_appointment);
    ...
    if (startTimes.includes(item)) {
      return (
        <View key={item} style={styles.itemContainer1}>
          <Text style={styles.text}>{`${item}:00 - ${item + duration}:00`}</Text>
        </View>
      );
    }
    return (
      <TouchableOpacity
        key={item}
        style={[styles.touchable, { backgroundColor: item === pressedHour ? "green" : "white" }]}
  ...
  return (
    <Modal animationType="fade" transparent={true} visible={isVisible} onRequestClose={onClose}>
      ...
      <View style={{ flex: 1, justifyContent: "center" }}>
        <FlatList data={hours} keyExtractor={(item) => item.toString()} renderItem={renderItem} />
       </View>
